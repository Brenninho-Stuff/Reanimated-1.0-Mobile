...
class Paths
{
	...
	public static var dumpExclusions:Array<String> = ['assets/shared/music/FreakyMenuSound.$SOUND_EXT'];
	...
	inline static public function sound(key:String, ?library:String = 'shared', ?useCurLevel:Bool = false, ?modsAllowed:Bool = true):Sound {
		if (useCurLevel == true) library = currentLevel;
		return returnSound('sounds/$key', library, modsAllowed);
	}

	inline static public function music(key:String, ?library:String = 'shared', ?useCurLevel:Bool = false, ?modsAllowed:Bool = true):Sound  {
		if (useCurLevel == true) library = currentLevel;
		return returnSound('music/$key', library, modsAllowed);
	}

	inline static public function musicAsString(key:String, ?library:String = 'shared', ?useCurLevel:Bool = false, ?modsAllowed:Bool = true):String {
		if (useCurLevel == true) library = currentLevel;
		return getPath('music/$key.$SOUND_EXT', MUSIC, library, modsAllowed);
	}

	inline static public function inst(song:String, ?library:String = 'songs', ?useCurLevel:Bool = false, ?modsAllowed:Bool = true):Sound {
		if (useCurLevel == true) library = currentLevel;
		return returnSound('${formatToSongPath(song)}/Inst', library, modsAllowed);
	}
	...
	inline static public function soundRandom(key:String, min:Int, max:Int, ?library:String = 'shared', ?useCurLevel:Bool = false, ?modsAllowed:Bool = true)
		return sound(key + FlxG.random.int(min, max), library, useCurLevel, modsAllowed);
	...
	inline static public function getTextFromFile(key:String, ?ignoreMods:Bool = false, ?library:String = 'shared'):String
	{
	...
	public static function loadAnimateAtlasFromLibrary(spr:FlxAnimate, folderOrImg:Dynamic, ?library:String = 'shared') {
		...
		if(Std.isOfType(folderOrImg, String))
		{
			...
			for (i in 0...10) {
				...
				if(i == 0) st = '';
				if(!changedAtlasJson) {
					spriteJson = getTextFromFile('images/$originalPath/spritemap$st.json', false, library);
					if(spriteJson != null) {
						changedImage = true;
						changedAtlasJson = true;
						folderOrImg = image('$originalPath/spritemap$st', library);
						break;
					}
				} else if(fileExists('images/$originalPath/spritemap$st.png', IMAGE, false, library)) {
					changedImage = true;
					folderOrImg = image('$originalPath/spritemap$st', library);
					break;
				}
			...
			if(!changedImage) {
				changedImage = true;
				folderOrImg = image(originalPath, library);
			}
			if(!changedAnimJson) {
				changedAnimJson = true;
				animationJson = getTextFromFile('images/$originalPath/Animation.json', false, library);
			}
		}
		spr.loadAtlasEx(folderOrImg, spriteJson, animationJson);
	}
	...
	public static function getDirectory(key:String, subFolder:String = 'images', ?library:String = 'shared') {
        ...
        if (FileSystem.exists('assets/$library$subFolder/$key')) {
			...
			trace('Could not find path: assets/$library$subFolder/$key');
			return '';
		}
	}

	public static function getDirectoryFiles(key:String, subFolder:String = 'images', ?library:String = 'shared') {
		var list = [];
		for (file in FileSystem.readDirectory(getDirectory(key, subFolder, library))) {
            list.push(file);
        }
		return list;
	}
